...
## Pending Tasks (Priority Order)
### 1. Iterative Pushover Analysis (New Method)
-   **Objective**: Create a specialized Pushover method that modifies the model iteratively.
-   **Concept**: 
    -   Run analysis -> Detect limit state (e.g., hinge) -> Change boundary condition/element property -> Continue/Restart.
    -   This allows simulating progressive collapse or component removal.
-   **Backend**: 
    -   Implement loop controlling OpenSees model state updates between analysis steps.

### 2. Moment-Curvature/Deformation Analysis
-   **Objective**: Visualize the behavior of sections at integration points.
-   **Frontend**: 
    -   Add a new Tab/Window to plot `Moment vs Curvature` (or Deformation) for specific elements/sections.
    -   Extract data from `eleResponse(tag, 'section', i, 'force')` and `('deformation')`.

### 3. Material Failure Limits
-   **Objective**: Allow models to actually "break".
-   **Implementation**: 
    -   Add `MinMax` material wrapper to `Steel01` in translator to simulate rupture.
    -   Expose rupture strain input in `MaterialDialog`.

## Technical Context for Next Session
-   **Current State**: Full system stable. Self-weight works. Pushover runs to completion (3m drift) without crashing.
-   **Next Steps**: Focus on advanced analysis features (Iterative, Section Inspection).
