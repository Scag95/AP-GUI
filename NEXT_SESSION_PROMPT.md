...
## Pending Tasks (Priority Order)
### 1. Element Force Debugging (High Priority)
-   **Objective**: Achieve strict equilibrium in element forces (zero moment at free tip).
-   **Plan**: 
    -   Compare `opensees_script.tcl` generated by GUI vs `test.py`.
    -   Tune `ForceBeamColumn` integration and iteration settings. 

### 1. Pushover Module Phase 2: Modal & Multi-Story (High Priority)
-   **Objective**: Implement "True" Pushover with load distribution proportional to the 1st Mode of Vibration.
-   **Backend**:
    -   Implement Eigenvalue analysis (`ops.eigen(1)`) in `OpenSeesTranslator` before adding lateral loads.
    -   Calculate Lateral Forces: $F_i = M_i \cdot \phi_{i,1}$ (Mass * Mode Shape).
    -   Normalize pattern relative to Control Node displacement.
    -   Implement **Story Shear Extraction**: Robust logic to calculate shear at each story (sum of column shears vs global reactions).
-   **Frontend**: 
    -   Plot **Story Capacity Curves**: (Story Shear vs Story Drift) for each floor, not just global Base Shear vs Roof Drift.
    -   Slider in `PushoverDialog` to visualize Deformation History.

## Technical Context for Next Session
-   **Estado Actual**: Pushover Analysis is fully functional and plots correct curves with correct unit labels. Element properties are editable.
-   **Next Steps**: Polish the physics (debugging free tip forces) and enhance Pushover Viz (Animation).
